...
static int btype_size(int bt)
{
  return bt == VT_BYTE || bt == VT_BOOL
             ? 1
             : bt == VT_SHORT ? 2 : bt == VT_INT ? 4 : bt == VT_LLONG ? 8 : bt == VT_PTR ? PTR_SIZE : 0;
}
...
ST_FUNC void tcc_debug_start(TCCState *s1)
{
  if (s1->do_debug) {
    ...
    put_stabs_r(s1, file->prev ? file->prev->filename : file->filename, N_SO, 0, 0, text_section->data_offset,
...
{
  if (type == N_LBRAC) {
    struct debug_info *info = (struct debug_info *)tcc_mallocz(sizeof(*info));
...
static void tcc_get_debug_info(TCCState *s1, Sym *s, CString *result)
{
  ...
  if ((type & VT_BTYPE) == VT_STRUCT) {
    ...
    if (debug_type == -1) {
      ...
      debug_hash = (struct debug_hash *)tcc_realloc(debug_hash, (n_debug_hash + 1) * sizeof(*debug_hash));
      debug_hash[n_debug_hash].debug_type = debug_type;
      debug_hash[n_debug_hash++].type = t;
      cstr_new(&str);
      cstr_printf(&str, "%s:T%d=%c%d",
                  (t->v & ~SYM_STRUCT) >= SYM_FIRST_ANOM ? "" : get_tok_str(t->v & ~SYM_STRUCT, NULL), debug_type,
                  IS_UNION(t->type.t) ? 'u' : 's', t->c);
      while (t->next) {
        int pos, size, align;

        t = t->next;
        cstr_printf(&str, "%s:", (t->v & ~SYM_FIELD) >= SYM_FIRST_ANOM ? "" : get_tok_str(t->v & ~SYM_FIELD, NULL));
    ...
    debug_type = ++debug_next_type;
    cstr_new(&str);
    cstr_printf(&str, "%s:T%d=e", (t->v & ~SYM_STRUCT) >= SYM_FIRST_ANOM ? "" : get_tok_str(t->v & ~SYM_STRUCT, NULL),
                debug_type);
    while (t->next) {
      t = t->next;
      cstr_printf(&str, "%s:", (t->v & ~SYM_FIELD) >= SYM_FIRST_ANOM ? "" : get_tok_str(t->v & ~SYM_FIELD, NULL));
...
static void tcc_debug_extern_sym(TCCState *s1, Sym *sym, int sh_num, int sym_bind)
{
  ...
  cstr_new(&str);
  cstr_printf(&str, "%s:%c", get_tok_str(sym->v, NULL), sym_bind == STB_GLOBAL ? 'G' : local_scope ? 'V' : 'S');
  tcc_get_debug_info(s1, sym, &str);
  if (sym_bind == STB_GLOBAL)
    tcc_debug_stabs(s1, str.data, N_GSYM, 0, NULL, 0);
  else
    tcc_debug_stabs(s1, str.data, (sym->type.t & VT_STATIC) && data_section == s ? N_STSYM : N_LCSYM, 0, s, sym->c);
  cstr_free(&str);
}
...
static void tcc_debug_typedef(TCCState *s1, Sym *sym)
{
  ...
  cstr_new(&str);
  cstr_printf(&str, "%s:t", (sym->v & ~SYM_FIELD) >= SYM_FIRST_ANOM ? "" : get_tok_str(sym->v & ~SYM_FIELD, NULL));
  tcc_get_debug_info(s1, sym, &str);
  tcc_debug_stabs(s1, str.data, N_LSYM, 0, NULL, 0);
  cstr_free(&str);
}
...
{
  BufferedFile *f;
  if (!s1->do_debug || cur_text_section != text_section || !(f = put_new_file(s1)) || last_line_num == f->line_num)
    return;
  if (func_ind != -1) {
...
ST_FUNC void vpush_global_sym(CType *type, int v) { vpushsym(type, external_global_sym(v, type)); }
...
/* find a free temporary local variable (return the offset on stack) match the size and align. If none, add new
...
static int gen_opic_lt(uint64_t a, uint64_t b) { return (a ^ (uint64_t)1 << 63) < (b ^ (uint64_t)1 << 63); }
...
/* return true if type1 and type2 are exactly the same (including
   qualifiers).
*/
static int is_compatible_types(CType *type1, CType *type2) { return compare_types(type1, type2, 0); }

/* return true if type1 and type2 are the same (ignoring qualifiers).
 */
static int is_compatible_unqualified_types(CType *type1, CType *type2) { return compare_types(type1, type2, 1); }

static void cast_error(CType *st, CType *dt) { type_incompatibility_error(st, dt, "cannot convert '%s' to '%s'"); }
...
redo:
  ...
  while (tok != ')') {
    ...
    case TOK_CLEANUP2: {
      ...
      s = sym_find(tok);
      if (!s) {
        tcc_warning("implicit declaration of function '%s'", get_tok_str(tok, &tokc));
...
ST_FUNC void subst_itp_by_fname(int tok_ident)
{
  ...
  // Push the function address of get_ptr_by_function_name
  sym.f.func_type = FUNC_NEW;
  sym.c = 0;
  type.ref = &sym;
  type.t = VT_LLONG;
  v.i = (uint64_t)tcci_state->redir.get_by_hash_sym->addr;
  vsetc(&type, VT_CONST, &v);
  vtop->r &= ~VT_LVAL; /* no lvalue */
  ...
  dba({
    ...
    printf("r:%i r2:%i\n", vtop->r, vtop->r2);
      ...
      //     if ((vtop->type.t & (VT_BTYPE | VT_ARRAY)) == VT_PTR) {
      //       vtop->type = *pointed_type(&vtop->type);
      //       printf("A.vtop[%li]->type.t:%i %li\n", vtop - vstack, vtop->type.t, vtop->c.i);
      //       printf("A.vtop->type.ref:%i\n", vtop->type.ref->);
      //       printf("A.r:%i r2:%i\n", vtop->r, vtop->r2);
      //       if ((vtop->type.t & VT_BTYPE) != VT_FUNC)
      //         goto error_func;
      //     }
  ...
  ret.r2 = VT_CONST;
  /* compute first implicit argument if a structure is returned */
  if ((was->type.t & VT_BTYPE) == VT_STRUCT) {
    variadic = (was->f.func_type == FUNC_ELLIPSIS);
    ret_nregs = gfunc_sret(&was->type, variadic, &ret.type, &ret_align, &regsize);
    if (ret_nregs <= 0) {
      /* get some space for the returned structure */
      size = type_size(&was->type, &align);
#ifdef TCC_TARGET_ARM64
      /* On arm64, a small struct is return in registers.
         It is much easier to write it to memory if we know
         that we are allowed to write some extra bytes, so
  ...
  // expect("aftgfunc");
  if (ret_nregs < 0) {
    dba(printf("ret.type.t:%i ret.r:%i r2:%i ret.c.i:%li\n", ret.type.t, ret.r, ret.r2, ret.c.i));
    vsetc(&ret.type, ret.r, &ret.c);
#ifdef TCC_TARGET_RISCV64
    arch_transfer_ret_regs(1);
#endif
    ...
    /* handle packed struct return */
    if (((was->type.t & VT_BTYPE) == VT_STRUCT) && ret_nregs) {
      int addr, offset;
      ...
      vset(&was->type, VT_LOCAL | VT_LVAL, addr);
...
tok_next:
  ...
  case TOK_builtin_constant_p:
    ...
    n = (vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST && !((vtop->r & VT_SYM) && vtop->sym->a.addrtaken);
    vtop--;
    vpushi(n);
    break;
