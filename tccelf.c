...
/* return a reference to a section, and create it if it does not
   exists */
ST_FUNC Section *find_section(TCCState *s1, const char *name) { return find_section_create(s1, name, 1); }
...
ST_FUNC int put_elf_sym(Section *s, addr_t value, unsigned long size, int info, int other, int shndx, const char *name)
{
  ...
  printf("put_elf_sym: '%s''%s' %lu\n", (name == NULL ? "(null)" : name), (char *)s->link->data + sym->st_name,
         sym->st_value);
  hs = s->hash;
  if (hs) {
    int *ptr, *base;
...
#if 1
        printf("new_bind=%x new_shndx=%x new_vis=%x old_bind=%x old_shndx=%x old_vis=%x\n", sym_bind, shndx, new_vis,
               esym_bind, esym->st_shndx, esym_vis);
#endif
...
ST_FUNC void put_stabs_r(TCCState *s1, const char *str, int type, int other, int desc, unsigned long value,
...
ST_FUNC void relocate_section(TCCState *s1, Section *s)
{
  ...
  for_each_elem(sr, 0, rel, ElfW_Rel)
  {
    ...
    printf("sym_index:%i-'%s' [%s] type:%i ptr:%p addr:%p sym->st_value:%lu tgt:%p\n", sym_index,
           (char *)s1->symtab->link->data + sym->st_name, s->name, type, ptr, (void *)addr, sym->st_value, (void *)tgt);
...
static struct sym_attr *put_got_entry(TCCState *s1, int dyn_reloc_type, int sym_index)
{
  ...
  if (need_plt_entry) {
    ...
    attr->plt_sym = put_elf_sym(s1->symtab, attr->plt_offset, sym->st_size, ELFW(ST_INFO)(STB_GLOBAL, STT_FUNC), 0,
...
ST_FUNC void tcc_add_btstub(TCCState *s1)
{
  ...
  cstr_new(&cstr);
  cstr_printf(&cstr, " extern void __bt_init(),*__rt_info[],__bt_init_dll();"
...
#endif
#endif
  cstr_printf(&cstr, "__bt_init(__rt_info,%d, 0);}", s1->output_type == TCC_OUTPUT_DLL ? 0 : s1->rt_num_callers + 1);
...
ST_FUNC int tcc_load_object_file(TCCState *s1, int fd, unsigned long file_offset)
{
  ...
  for (i = 1; i < ehdr.e_shnum; i++) {
    ...
    if (sh->sh_type != SHT_PROGBITS &&
        ...
        sh->sh_type != SHT_FINI_ARRAY && strcmp(strsec + sh->sh_name, ".stabstr"))
      continue;
    if (seencompressed && !strncmp(strsec + sh->sh_name, ".debug_", sizeof(".debug_") - 1))
      continue;
