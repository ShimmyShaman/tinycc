...
tail_call:
  ...
  fprintf(stderr, "limit=%5d, size=%5g MB, nb_peak=%6d, nb_total=%8d, nb_missed=%6d, usage=%5.1f%%\n", al->limit,
          al->size / 1024.0 / 1024.0, al->nb_peak, al->nb_total, al->nb_missed,
...
static int expr_preprocess(void)
{
  ...
  redo:
    if (tok == TOK_DEFINED) {
      ...
      next();
      if (tok == '(')
        tcc_error("function-like macro '%s' is not defined", get_tok_str(t, NULL));
      goto redo;
    }
    tok_str_add_tok(str);
  }
...
static void tcc_predefs(CString *cstr)
{
  cstr_cat(cstr,
        ...
        //    "#include <tcc_predefs.h>\n"
           ...
           "#define __builtin_va_arg(ap,type) "
           "(*(sizeof(type)>(2*__va_reg_size)?*(type**)((ap+=__va_reg_size)-__va_reg_size):(ap=(va_list)(_tcc_align(ap,"
           "type)+(sizeof(type)+__va_reg_size-1)&-__va_reg_size),(type*)(ap-((sizeof(type)+__va_reg_size-1)&-__va_reg_"
           "size)))))\n"
