...
ST_DATA const int reg_classes[NB_REGS] = {
    /* eax */ RC_INT | RC_RAX,
    /* ecx */ RC_INT | RC_RCX,
    /* edx */ RC_INT | RC_RDX, 0, 0, 0, 0, 0, RC_R8, RC_R9, RC_R10, RC_R11, 0, 0, 0, 0,
...
static void gen_gotpcrel(int r, Sym *sym, int c)
{
  ...
  tcc_error("internal error: no GOT on PE: %s %x %x | %02x %02x %02x\n", get_tok_str(sym->v, NULL), c, r,
            cur_text_section->data[ind - 3], cur_text_section->data[ind - 2], cur_text_section->data[ind - 1]);
#endif
...
/* generate a modrm reference. 'op_reg' contains the additional 3
   opcode bits */
static void gen_modrm(int op_reg, int r, Sym *sym, int c) { gen_modrm_impl(op_reg, r, sym, c, 0); }
...
void gfunc_call(int nb_args)
{
  ...
  for (i = 0; i < nb_args; i++) {
    ...
    // printf("AFTER: nb_args=%i size=%i align=%i reg_count=%i\n", nb_args, size, align, reg_count);
    if (size == 0)
      continue;
...
ST_FUNC void gen_vla_sp_restore(int addr) { gen_modrm64(0x8b, TREG_RSP, VT_LOCAL, NULL, addr); }
