...
#define CONFIG_TCC_LIBPATHS                                           \
  ...
  ":" ALSO_TRIPLET(CONFIG_SYSROOT "/" CONFIG_LDDIR) ":" ALSO_TRIPLET( \
      CONFIG_SYSROOT                                                  \
      "/usr/local/" CONFIG_LDDIR) ":" ALSO_TRIPLET(CONFIG_SYSROOT "/usr/" CONFIG_LDDIR "/x86_64-linux-gnu")
#endif
#endif
...
struct SymAttr {
  unsigned short aligned : 5, /* alignment as log2+1 (0 == unspecified) */
      packed : 1, weak : 1, visibility : 2, dllexport : 1, nodecorate : 1, dllimport : 1, addrtaken : 1,
      xxxx : 3; /* not used */
};
...
static inline int is_space(int ch) { return ch == ' ' || ch == '\t' || ch == '\v' || ch == '\f' || ch == '\r'; }
static inline int isid(int c) { return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_'; }
...
static inline uint32_t read32le(unsigned char *p) { return read16le(p) | (uint32_t)read16le(p + 2) << 16; }
...
static inline uint64_t read64le(unsigned char *p) { return read32le(p) | (uint64_t)read32le(p + 4) << 32; }
